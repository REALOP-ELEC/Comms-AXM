...
__reentrantb uint32_t crc_buf(const uint8_t __genericaddr *buf, uint16_t buflen, uint32_t crc) __reentrant __naked
{
	buf;
	buflen;
	crc;
...
__reentrantb uint32_t crc_buf(const uint8_t __genericaddr *buf, uint16_t buflen, uint32_t crc) __reentrant
{
	if (!buflen)
		return crc;
	do {
