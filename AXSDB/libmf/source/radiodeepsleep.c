...
{
#if RADIO == 5043
	GPIO_PRT2->DR |= 0x0A; /* MISO, SEL, RSYSCLK */
	radio_write8(AX5043_REG_PINFUNCSYSCLK, 0x01);
#elif RADIO == 5044 /* or 5045 */
	GPIO_PRT2->DR |= 0x0A; /* MISO, SEL, RSYSCLK */
	radio_write8(AX5044_45_REG_PINFUNCSYSCLK, 0x01);
#else
...
#if RADIO == 5043
    GPIO_OR->DATA_OUT = 0x0B000000;               //  Set SEL as High (At the rising edge of the SEL line, the chip will enter deep sleep mode)
    radio_write8(AX5043_REG_PINFUNCSYSCLK, 0x01); //  SYSCLK Output ‘1’
#elif RADIO == 5044 /* or 5045 */
    GPIO_OR->DATA_OUT = 0x0B000000;               //  Set SEL as High (At the rising edge of the SEL line, the chip will enter deep sleep mode)
    radio_write8(AX5044_45_REG_PINFUNCSYSCLK, 0x01); //  SYSCLK Output ‘1’
#else
