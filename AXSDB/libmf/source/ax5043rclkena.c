...
__reentrantb void ax5043_rclk_enable(uint8_t div) __reentrant
{
	...
	div += 4;
	crit = enter_critical();
	radio_write8(AX5043_REG_PINFUNCSYSCLK, div);
#if defined __ARMEL__ || defined __ARMEB__
#ifdef __AXM0
    ...
    SET_BIT_FIELDS(GPIO_PRT3->PC, 0x00000038, 0x00000008);
#endif /* __AXM0F2 */
#else
	PORTR &= (uint8_t)~0x02;
#endif

	p = radio_read8(AX5043_REG_PWRMODE);
	if (!(p & 0x0F))
    {
        radio_write8(AX5043_REG_PWRMODE, (p & 0xF0) | 0x05);
    }
    exit_critical(crit);
}
