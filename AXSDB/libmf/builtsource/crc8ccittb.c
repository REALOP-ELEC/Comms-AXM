...
__reentrantb uint8_t crc_buf(const uint8_t __genericaddr *buf, uint16_t buflen, uint8_t crc) __reentrant __naked
{
	buf;
	buflen;
	crc;
...
__reentrantb uint8_t crc_buf(const uint8_t __genericaddr *buf, uint16_t buflen, uint8_t crc) __reentrant
{
	if (!buflen)
		return crc;
	do {
